              ...
              <div className="flex flex-col items-center gap-3 py-2 text-center">
                ...
                {!modal.payment_method && (
                  <div className="w-full space-y-2">
                    <p className="text-sm font-semibold text-gray-700">¿Cómo vas a pagar?</p>
                    {(modal.route.nequi_qr_key || modal.route.nequi_phone) && (
                      <button
                        onClick={() => setModal((m) => m ? { ...m, payment_method: 'nequi' } : m)}
                        className="w-full flex items-center gap-3 bg-[#7B2D8B]/5 border-2 border-[#7B2D8B]/30 hover:border-[#7B2D8B]/60 hover:bg-[#7B2D8B]/10 rounded-xl px-4 py-3 transition-colors text-left"
                      >
                        <span className="text-2xl">💜</span>
                        <div>
                          <p className="text-sm font-bold text-[#7B2D8B]">Pagar por Nequi</p>
                          <p className="text-xs text-gray-500">Transfiere al conductor antes de subir</p>
                        </div>
                      </button>
                    )}
                    <button
                      onClick={() => setModal((m) => m ? { ...m, payment_method: 'cash' } : m)}
                      className="w-full flex items-center gap-3 bg-amber-50 border-2 border-amber-200 hover:border-amber-400 hover:bg-amber-100 rounded-xl px-4 py-3 transition-colors text-left"
                    >
                      <span className="text-2xl">🤝</span>
                      <div>
                        <p className="text-sm font-bold text-amber-700">Le pago en persona</p>
                        <p className="text-xs text-gray-500">Pagas directamente al conductor al subir</p>
                      </div>
                    </button>
                  </div>
                )}

                {/* Bloque pago en persona */}
                {modal.payment_method === 'cash' && (
                  <div className="w-full bg-amber-50 border border-amber-300 rounded-xl p-4 space-y-2 text-left">
                    <div className="flex items-start gap-2">
                      <span className="text-xl shrink-0">⚠️</span>
                      <div>
                        <p className="text-sm font-bold text-amber-800">Recuerda honrar tu reserva</p>
                        <p className="text-xs text-amber-700 mt-1">
                          El conductor irá hasta tu ubicación confiando en que vas a pagar. Si no apareces o cancelas sin avisar, le haces perder tiempo y dinero. Por favor sé responsable.
                        </p>
                      </div>
                    </div>
                    {(modal.route.whatsapp || modal.route.phone) && (
                      <div className="pt-1">
                        <p className="text-xs text-gray-500 mb-1.5">Avísale al conductor que ya estás listo:</p>
                        {modal.route.whatsapp ? (
                          <a
                            href={`https://wa.me/57${modal.route.whatsapp.replace(/\D/g, '')}?text=${encodeURIComponent(`Hola ${modal.route.full_name}, reservé un puesto en tu ruta hacia ${modal.destination}. Te pagaré en persona al subir.`)}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="flex items-center justify-center gap-2 bg-[#25D366]/10 text-[#128C7E] font-semibold px-3 py-2 rounded-lg hover:bg-[#25D366]/20 transition-colors text-sm w-full"
                          >
                ...
                {/* Bloque pago Nequi */}
                {modal.payment_method === 'nequi' && (
                  <div className="w-full bg-[#7B2D8B]/5 border border-[#7B2D8B]/20 rounded-xl p-4 space-y-3">
                    <p className="text-xs font-semibold text-[#7B2D8B] text-center">Paga con Nequi para asegurar tu puesto</p>

                    {modal.route.nequi_phone && (
                      <div className="flex items-center gap-2 bg-white border border-[#7B2D8B]/20 rounded-lg px-3 py-2">
                        <span className="text-sm text-gray-500 shrink-0">Número:</span>
                        <span className="text-sm font-bold text-gray-900 flex-1">{modal.route.nequi_phone}</span>
                        <button
                          onClick={() => navigator.clipboard.writeText(modal.route.nequi_phone!)}
                          className="text-xs text-[#7B2D8B] font-semibold hover:underline shrink-0"
                        >
                          Copiar
                        </button>
                      </div>
                    ...
                    {modal.route.nequi_qr_key && (
                      <>
                        <img
                          ...
                          className="w-44 h-44 object-contain rounded-xl mx-auto border border-[#7B2D8B]/10"
                        />
                        <p className="text-xs text-gray-500 text-center">Escanea con la app de Nequi o Bancolombia</p>
                        <button
                          onClick={async () => {
                    ...
                    {(modal.route.whatsapp || modal.route.phone) && (
                      <div className="pt-1 space-y-1">
                        <p className="text-xs text-gray-500">Envíale el comprobante al conductor:</p>
                        {modal.route.whatsapp ? (
                          <a
                            href={`https://wa.me/57${modal.route.whatsapp.replace(/\D/g, '')}?text=${encodeURIComponent(`Hola ${modal.route.full_name}, reservé un puesto en tu ruta hacia ${modal.destination}. Te envío el comprobante de pago por Nequi.`)}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="flex items-center justify-center gap-2 bg-[#25D366]/10 text-[#128C7E] font-semibold px-3 py-2 rounded-lg hover:bg-[#25D366]/20 transition-colors text-sm w-full"
                          >
                ...
                {modal.payment_method && (
                  <button
                    onClick={() => setModal((m) => m ? { ...m, payment_method: undefined } : m)}
                ...
                <button
                  ...
                  className="w-full bg-[#008000] text-white font-semibold py-2.5 rounded-xl"
                >
                  Entendido
                </button>
                {modal.requestId && (
