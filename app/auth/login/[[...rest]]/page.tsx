...
export default function LoginPage() {
  ...
  useEffect(() => { signInRef.current = signIn; isLoadedRef.current = isLoaded; }, [signIn, isLoaded]);
  ...
  const handleGoogleLogin = async () => {
    ...
    setError("");
    // Esperar hasta 3s leyendo los refs (que sí se actualizan)
    let attempts = 0;
    while ((!isLoadedRef.current || !signInRef.current) && attempts < 30) {
    ...
    >
      ...
      <div className="flex items-center justify-center min-h-[calc(100vh-4rem)] px-4 py-8">
        <div className="max-w-md w-full space-y-8 bg-white bg-opacity-90 p-8 rounded-2xl shadow-2xl border border-[#008000] border-opacity-30">
          ...
          {notRegistered && (
            <div className="bg-[#008000]/10 border-l-4 border-[#008000] text-[#003300] px-4 py-4 rounded-lg shadow-md">
              <div className="flex items-start gap-3">
                <svg className="w-5 h-5 flex-shrink-0 mt-0.5 text-[#008000]" fill="currentColor" viewBox="0 0 20 20">
