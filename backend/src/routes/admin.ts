...
adminRoutes.delete('/users/:id', async (c) => {
  try {
    ...
    const tables: Array<{ sql: string; params: string[] }> = [
      ...
      { sql: 'DELETE FROM driver_of_month WHERE driver_id = ?', params: [userId] },
      { sql: 'DELETE FROM driver_price_offers WHERE driver_id = ?', params: [userId] },
      ...
      { sql: 'DELETE FROM web_push_subscriptions WHERE user_id = ?', params: [userId] },
      { sql: 'DELETE FROM phone_verifications WHERE phone = (SELECT phone FROM users WHERE id = ?)', params: [userId] },
