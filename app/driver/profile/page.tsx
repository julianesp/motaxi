...
export default function DriverProfilePage() {
  ...
  // Fotos del vehículo
  const [vehiclePhotos, setVehiclePhotos] = useState<DriverPhoto[]>([]);
  const [isUploadingVehiclePhoto, setIsUploadingVehiclePhoto] = useState(false);
  const [selectedVehiclePhoto, setSelectedVehiclePhoto] = useState<File | null>(null);
  const [vehiclePhotoPreview, setVehiclePhotoPreview] = useState<string | null>(null);
  const [vehiclePhotoCaption, setVehiclePhotoCaption] = useState('');

  interface DriverStats {
    route_history: { id: string; origin: string; destination: string; created_at: number; total_seats: number; passenger_count: number; estimated_earnings: number; status: string }[];
    total_routes: number;
    total_earnings: number;
    frequent_passengers: { passenger_name: string; passenger_phone: string; trip_count: number }[];
  ...
  const fetchVehiclePhotos = async () => {
    try {
      const { apiClient } = await import('@/lib/api-client');
      const res = await apiClient.get('/drivers/vehicle-photos/my');
  ...
  const handleUploadVehiclePhoto = async () => {
    if (!selectedVehiclePhoto) return;
    setIsUploadingVehiclePhoto(true);
    try {
      const API_URL = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8787';
      const token = document.cookie.match(/authToken=([^;]+)/)?.[1];
      const fd = new FormData();
      fd.append('photo', selectedVehiclePhoto);
      if (vehiclePhotoCaption.trim()) fd.append('caption', vehiclePhotoCaption.trim());
      const res = await fetch(`${API_URL}/drivers/vehicle-photos`, {
        method: 'POST',
      ...
      setVehiclePhotoCaption('');
      await fetchVehiclePhotos();
      Swal.fire({ icon: 'success', title: 'Foto agregada', text: 'La foto de tu vehículo ya aparece en tu perfil público.', confirmButtonColor: '#008000' });
    } catch (error: any) {
  ...
  const handleDeleteVehiclePhoto = async (photoId: string) => {
    const result = await Swal.fire({
      title: '¿Eliminar foto del vehículo?',
      icon: 'warning',
      showCancelButton: true,
    ...
    try {
      const { apiClient } = await import('@/lib/api-client');
      await apiClient.delete(`/drivers/vehicle-photos/${photoId}`);
      await fetchVehiclePhotos();
    } catch {
          ...
          <div className="mt-6">
            ...
            <div className="mt-3 bg-white rounded-xl shadow-md p-4">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="font-semibold text-gray-800">Invita pasajeros y gana un mes gratis</p>
                  <p className="text-xs text-gray-500">El conductor que más pasajeros registre con su enlace no paga el siguiente mes.</p>
                </div>
              </div>
              ...
              {referralStats !== null && (
                <div className="grid grid-cols-3 gap-2 mb-3">
                  <div className="bg-[#008000]/5 rounded-xl p-3 text-center">
                    <p className="text-xl font-bold text-[#008000]">{referralStats.this_month}</p>
                  ...
                  <div className="bg-amber-50 rounded-xl p-3 text-center">
                    <p className="text-xl font-bold text-amber-600">#{referralStats.rank}</p>
              ...
              <div className="flex items-center gap-2">
                <div className="flex-1 bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-xs text-gray-600 truncate">
                  {typeof window !== 'undefined' ? `${window.location.origin}/auth/register?ref=${user?.id}` : `motaxi.app/auth/register?ref=${user?.id}`}
                </div>
                <button
                  onClick={async () => {
                    const link = `${window.location.origin}/auth/register?ref=${user?.id}`;
                    if (navigator.share) {
                      navigator.share({ title: 'Únete a MoTaxi', text: 'Regístrate como pasajero en MoTaxi con mi enlace', url: link });
                    } else {
                  ...
                  className="flex-shrink-0 flex items-center gap-1.5 bg-[#008000] text-white text-xs font-semibold px-3 py-2 rounded-lg hover:bg-[#006800] transition-colors"
                >
                  {referralLinkCopied ? (
            ...
            <div className="mt-3 bg-white rounded-xl shadow-md p-4">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">Aparecen en tu perfil público para que los pasajeros conozcan tu vehículo. Máx. 10 fotos.</p>
                </div>
              </div>
              ...
              {!vehiclePhotoPreview ? (
                <label className="flex items-center justify-center w-full border-2 border-dashed border-[#008000]/40 rounded-xl p-5 cursor-pointer hover:border-[#008000] hover:bg-[#008000]/5 transition-colors">
                  <div className="text-center">
                    ...
                    <p className="text-sm text-[#008000] font-medium">Agregar foto del vehículo</p>
                    <p className="text-xs text-gray-400 mt-1">JPG, PNG · máx. 5MB</p>
                  </div>
                  <input type="file" accept="image/*" className="hidden" onChange={handleVehiclePhotoSelect} />
                </label>
              ) : (
                <div className="space-y-3">
                  <div className="relative rounded-xl overflow-hidden">
                    <img src={vehiclePhotoPreview} alt="Vista previa" className="w-full h-48 object-cover" />
                    <button
                      onClick={() => { setVehiclePhotoPreview(null); setSelectedVehiclePhoto(null); }}
                  ...
                  <input
                    type="text"
                    placeholder="Descripción (opcional, ej: Vista lateral)"
                    value={vehiclePhotoCaption}
                    onChange={e => setVehiclePhotoCaption(e.target.value)}
                    maxLength={120}
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#008000]"
                  />
                  <button
                    onClick={handleUploadVehiclePhoto}
              ...
              {vehiclePhotos.length === 0 && !vehiclePhotoPreview && (
                <p className="text-center text-xs text-gray-400 mt-3">Aún no tienes fotos de tu vehículo</p>
              )}
            </div>
            ...
            <div className="mt-6 space-y-4">
              <h2 className="text-base font-bold text-gray-800 px-1">Mi actividad</h2>

              {/* Insignias */}
              <div className="bg-white rounded-xl shadow-md p-4">
                <p className="text-sm font-semibold text-gray-700 mb-3">Insignias</p>
                {statsLoading ? (
                  <p className="text-xs text-gray-400">Cargando...</p>
                ) : (
                  <div className="flex flex-wrap gap-2">
                    {(driverStats?.badges || []).map((b) => (
                      <span
                        key={b.id}
                        className={`flex items-center gap-1 text-xs font-medium px-2.5 py-1.5 rounded-full border ${b.earned ? 'bg-[#008000]/10 border-[#008000]/30 text-[#008000]' : 'bg-gray-100 border-gray-200 text-gray-400 opacity-50'}`}
                      >
                        {b.icon} {b.label}
                      </span>
                    ))}
              ...
              {/* Ganancias estimadas */}
              <div className="bg-white rounded-xl shadow-md p-4">
                <p className="text-sm font-semibold text-gray-700 mb-3">Ganancias estimadas (rutas)</p>
                {statsLoading ? (
                  <p className="text-xs text-gray-400">Cargando...</p>
                ) : (
                  <div className="flex gap-4">
                    <div className="flex-1 bg-[#008000]/5 rounded-xl p-3 text-center">
                      <p className="text-2xl font-bold text-[#008000]">${(driverStats?.total_earnings || 0).toLocaleString('es-CO')}</p>
                      <p className="text-xs text-gray-500 mt-0.5">Total acumulado</p>
                    </div>
                    <div className="flex-1 bg-gray-50 rounded-xl p-3 text-center">
                      <p className="text-2xl font-bold text-gray-700">{driverStats?.total_routes || 0}</p>
                      <p className="text-xs text-gray-500 mt-0.5">Rutas publicadas</p>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="bg-white rounded-xl shadow-md p-4 space-y-3">
                <p className="text-sm font-semibold text-gray-700">Horario y ruta habitual</p>
                <p className="text-xs text-gray-400">Los pasajeros verán esta info en tu tarjeta.</p>
                <div>
                  <label className="text-xs font-medium text-gray-500 block mb-1">Horarios habituales</label>
                  <input
                    type="text"
                    placeholder="Ej: Lunes a viernes 6am–8am, 5pm–7pm"
                    value={driverFormData.usual_hours}
                    onChange={(e) => setDriverFormData((p) => ({ ...p, usual_hours: e.target.value }))}
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#008000]/30"
                  />
                </div>
                <div className="flex gap-2">
                  <div className="flex-1">
                    <label className="text-xs font-medium text-gray-500 block mb-1">Origen habitual</label>
                    <input
                      type="text"
                      placeholder="Ej: Colón"
                      value={driverFormData.usual_origin}
                      onChange={(e) => setDriverFormData((p) => ({ ...p, usual_origin: e.target.value }))}
                      className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#008000]/30"
                    />
                  </div>
                  <div className="flex-1">
                    <label className="text-xs font-medium text-gray-500 block mb-1">Destino habitual</label>
                    <input
                      type="text"
                      placeholder="Ej: Sibundoy"
                      value={driverFormData.usual_destination}
                      onChange={(e) => setDriverFormData((p) => ({ ...p, usual_destination: e.target.value }))}
                      className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#008000]/30"
                    />
                  </div>
                </div>
                  ...
                  onClick={async () => {
                    try {
                      ...
                      await driversAPI.updateProfile({
                        usual_hours: driverFormData.usual_hours,
                        usual_origin: driverFormData.usual_origin,
                        usual_destination: driverFormData.usual_destination,
                      });
                      Swal.fire({ icon: 'success', title: 'Guardado', confirmButtonColor: '#008000', timer: 1500, showConfirmButton: false });
                    } catch {
                      Swal.fire({ icon: 'error', title: 'Error', text: 'No se pudo guardar.', confirmButtonColor: '#008000' });
                    }
                  }}
                  className="w-full bg-[#008000] text-white text-sm font-semibold py-2 rounded-lg"
                >
                  Guardar horario y ruta
                </button>
              </div>

              {/* Pasajeros frecuentes */}
              {(driverStats?.frequent_passengers?.length ?? 0) > 0 && (
                <div className="bg-white rounded-xl shadow-md p-4">
                  <p className="text-sm font-semibold text-gray-700 mb-3">Pasajeros frecuentes</p>
                  <div className="space-y-2">
                    {driverStats!.frequent_passengers.map((p) => (
                      <div key={p.passenger_phone} className="flex items-center justify-between gap-3">
                        <div className="flex items-center gap-2 min-w-0">
                          <div className="w-8 h-8 rounded-full bg-[#008000]/10 flex items-center justify-center shrink-0">
                            <span className="text-xs font-bold text-[#008000]">{p.passenger_name?.charAt(0)?.toUpperCase()}</span>
                          </div>
                          <div className="min-w-0">
                            <p className="text-sm font-medium text-gray-800 truncate">{p.passenger_name}</p>
                            <p className="text-xs text-gray-400">{p.trip_count} viajes contigo</p>
                          </div>
                        </div>
                        <a
                          href={`https://wa.me/57${p.passenger_phone.replace(/\D/g, '')}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center gap-1 bg-[#25D366]/10 text-[#128C7E] text-xs font-semibold px-2.5 py-1.5 rounded-lg shrink-0"
                        >
              ...
              {/* Historial de rutas */}
              {(driverStats?.route_history?.length ?? 0) > 0 && (
                <div className="bg-white rounded-xl shadow-md p-4">
                  <p className="text-sm font-semibold text-gray-700 mb-3">Últimas rutas</p>
                  <div className="space-y-2">
                    {driverStats!.route_history.map((r) => (
                      <div key={r.id} className="flex items-center justify-between gap-2 py-2 border-b border-gray-100 last:border-0">
                        <div className="min-w-0">
                          <p className="text-sm font-medium text-gray-800">{r.origin} → {r.destination}</p>
                          <p className="text-xs text-gray-400">
                            {new Date(r.created_at * 1000).toLocaleDateString('es-CO', { day: 'numeric', month: 'short' })}
                            {' · '}{r.passenger_count}/{r.total_seats} pasajeros
                          </p>
                        </div>
                        <span className="text-sm font-semibold text-[#008000] shrink-0">
                          ${(r.passenger_count * (r.estimated_earnings / Math.max(r.passenger_count, 1))).toLocaleString('es-CO') || '0'}
                        </span>
                      </div>
                    ))}
                  </div>
