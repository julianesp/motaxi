...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const response = await fetch(`${API_URL}/admin/demand-alerts/run`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      console.error('[cron demand-alerts] Error del backend:', result);
      return NextResponse.json({ error: 'Error en el backend', details: result }, { status: 500 });
    }

    console.log('[cron demand-alerts] Ejecutado:', result);
    return NextResponse.json({ success: true, ...result });
  } catch (error: any) {
    console.error('[cron demand-alerts] Error:', error);
